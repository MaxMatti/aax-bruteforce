...
void worker_thread(char* hash, std::uint64_t counter,
                   std::uint64_t counter_end) {
  const char* constant =
  ...
  for (; counter < counter_end; ++counter) {
    ...
    if (memcmp(stage3, hash, 20) == 0) {
      std::cout << "Activation bytes: " << helpers::base16(bytes, 4)
                << std::endl;
      break;
    }
  }
...
int main(int argc, char const* argv[]) {
  ...
  for (int i = 0; i < 20; ++i) {
    hash[i] =
        ((hash_base16[i * 2] + (hash_base16[i * 2] & 0x40 ? 9 : 0)) << 4) |
        ((hash_base16[i * 2 + 1] + (hash_base16[i * 2 + 1] & 0x40 ? 9 : 0)) &
         0xf);
  }
  ...
  for (unsigned int i = 0; i < threadcount; ++i) {
    threads.emplace_back(worker_thread, hash, keyspace * i, keyspace * (i + 1));
  }
  for (auto& t : threads) {
    t.join();
  }
